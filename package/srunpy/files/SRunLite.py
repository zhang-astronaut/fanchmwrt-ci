...
class SrunClient():
    def __init__(self, srun_host='gw.buaa.edu.cn',protcol='https',verify=True):
        if '/' in srun_host:
            raise Exception('srun_host should be a domain name or an ip address')
        if protcol not in ['http', 'https']:
            raise Exception('protcol should be http or https')
        self.verify = verify
        self.srun_host = srun_host
        self.init_url = f"{protcol}://{srun_host}"
        self.get_ip_api = f'{protcol}://{srun_host}/cgi-bin/rad_user_info?callback=JQuery'
        self.get_challenge_api = f"{protcol}://{srun_host}/cgi-bin/get_challenge"
        self.srun_portal_api = f"{protcol}://{srun_host}/cgi-bin/srun_portal"
        self.rad_user_dm_api = f"{protcol}://{srun_host}/cgi-bin/rad_user_dm"
    ...
    def init_getip(self):
        res = requests.get(self.get_ip_api,verify=self.verify)
    ...
    def get_token(self,username,ip):
        ...
        get_challenge_res = test.get(self.get_challenge_api, params=get_challenge_params, headers=self.header,verify=self.verify)
    ...
    def login(self,username,password):
        ...
        test = requests.Session()
        srun_portal_res = test.get(self.srun_portal_api, params=srun_portal_params, headers=self.header,verify=self.verify)
    ...
    def logout(self):
        ...
        test = requests.Session()
        user_dm_res = test.get(self.rad_user_dm_api, params=user_dm_params, headers=self.header,verify=self.verify)
